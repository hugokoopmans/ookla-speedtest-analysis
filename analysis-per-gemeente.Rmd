...
Gemeente `r df.top.plus.unknown$gm_naam[i]`
-----------------
The table shows the speed test analysis for gemeente `r df.top.plus.unknown$gm_naam[i]`. For this test the confidence level is set to `r 100*conflevel`%.

``` {r echo=FALSE, cache=FALSE, results='asis'}

# create temp df
# filter specific gemeente
df.tmp <- subset(df.4G.plus.hotspots,gm_naam == df.top.plus.unknown$gm_naam[i])
