...
Introduction
===========
This document is a report of a statistical analysis of 4G network speed test data. The time period we consider is Q1 2015 so the months January, February and March of 2015 are in scope. 
...
con <- dbConnect(drv, dbname="4Gdb",host="localhost",port=5432,user="hugo",password="hugo" )
...
The raw data
---------------
We load the raw files, downloaded from the Ookla server, into individual tables per mobile platform. In the table below we count the number of speed tests per mobile platform in Q1/2015.
...
drv <- dbDriver("PostgreSQL")
# Full version of connection setting
con <- dbConnect(drv, dbname="4Gdb",host="localhost",port=5432,user="hugo",password="hugo" ) #TO DO USER AND PASSWORD in CONFIGURATIE FILE
...
We identify `r sum(tbl.oct>=max_susp_dev)` devices in January, `r sum(tbl.nov>=max_susp_dev)` devices in February and `r sum(tbl.dec>=max_susp_dev)` devices in March, which in total represent `r nrow(df.ad)-nrow(df.clnd)` speed tests.
...
Areas where T-Mobile announced 4G coverage in Q1 2015 T-Mobile announced 4G coverage in : Buren, Culemborg, Geldermalsen, Scherpenzeel, Tiel, Neerijnen, Renswoude, Wijk bij Duurstede, Sint-Michielsgestel, Utrechtse Heuvelrug, Maasdonk, Barneveld, Ede, Veenendaal, Hellevoetsluis, Bernisse, Korendijk, Cromstrijen, Westvoorne, Strijen, Moerdijk, Goeree-Overflakkee, Almelo, Borne, Deventer, Hengelo, Wierden, Zwolle, Beuningen, Druten, Ermelo, Harderwijk, Hattem, Lochem, Putten, Voorst, Wageningen, Wijchen, Zutphen, Nunspeet, Rhenen, West Maas en Waal, Oss, Overbetuwe, Neder-Betuwe, Rijssen-Holten.
...
We find no disturbing or unknown peaks on a specific date.  #TO DO Validate Dejan
